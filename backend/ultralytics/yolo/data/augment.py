...
class Mosaic(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        for i in range(4):
            ...
            clean_img = labels_patch.get('clean_img')
            h, w = labels_patch.pop('resized_shape')

            # place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                clean4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8) if clean_img is not None else None
            ...
            if clean_img is not None and clean4 is not None:
                clean4[y1a:y2a, x1a:x2a] = clean_img[y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
        ...
        if 'clean4' in locals() and clean4 is not None:
            final_labels['clean_img'] = clean4
        return final_labels
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        final_labels = {
            ...
            'clean_file': mosaic_labels[0].get('clean_file'),
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        if 'clean_img' in labels and 'clean_img' in labels2:
            labels['clean_img'] = (labels['clean_img'] * r + labels2['clean_img'] * (1 - r)).astype(np.uint8)
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        if 'clean_img' in labels:
            if self.perspective:
                labels['clean_img'] = cv2.warpPerspective(labels['clean_img'], M, dsize=self.size,
                                                          borderValue=(114, 114, 114))
            else:
                labels['clean_img'] = cv2.warpAffine(labels['clean_img'], M[:2], dsize=self.size,
                                                     borderValue=(114, 114, 114))
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == 'vertical' and random.random() < self.p:
            img = np.flipud(img)
            if 'clean_img' in labels:
                labels['clean_img'] = np.flipud(labels['clean_img'])
            instances.flipud(h)
        if self.direction == 'horizontal' and random.random() < self.p:
            img = np.fliplr(img)
            if 'clean_img' in labels:
                labels['clean_img'] = np.fliplr(labels['clean_img'])
            instances.fliplr(w)
        labels['img'] = np.ascontiguousarray(img)
        if 'clean_img' in labels:
            labels['clean_img'] = np.ascontiguousarray(labels['clean_img'])
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if 'clean_img' in labels:
            clean_img = labels['clean_img']
            if shape[::-1] != new_unpad:
                clean_img = cv2.resize(clean_img, new_unpad, interpolation=cv2.INTER_LINEAR)
            labels['clean_img'] = cv2.copyMakeBorder(clean_img, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                                     value=(114, 114, 114))
...
class Format:
    ...
    def __call__(self, labels):
        img = labels.pop('img')
        clean_img = labels.pop('clean_img', None)
        h, w = img.shape[:2]
        cls = labels.pop('cls')
        instances = labels.pop('instances')
        ...
        if clean_img is not None:
            labels['clean_img'] = self._format_img(clean_img)
